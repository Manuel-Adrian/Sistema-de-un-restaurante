<?php
	...
	$sql = "SELECT * FROM mesa1 WHERE status = 'EN ESPERA' ORDER BY id DESC";
	$resultado = $mysqli->query($sql);

	$query = "SELECT COUNT(status) AS contador FROM mesa1 WHERE status = 'PREPARANDO'";
	$contador_status = $mysqli->query($query);

	$consulta = "SELECT COUNT(status) AS contador FROM mesa1 WHERE status = 'PREPARANDO'";
	$contador = $mysqli->query($consulta);

	$query1 = "SELECT SUM(subtotal) as totall FROM mesa1 WHERE status = 'EN ESPERA'";
					...
					<center>
						...
						<div class="modal-body">
							<div class="container-fluid">
								<form method="POST" name="formulario1" action="guardar_mesa1.php">
